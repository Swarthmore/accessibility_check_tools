...
aws_region=$(sed -n 's/aws_region = "\(.*\)"/\1/p' terraform/terraform.tfvars)

# Add deploy suffix to S3 bucket (if present)
s3_deploy_bucket=$(sed -n 's/deploy_s3_bucket = "\(.*\)"/\1/p' terraform/terraform.tfvars)
deploy_suffix=$(sed -n 's/deploysuffix = "\(.*\)"/\1/p' terraform/terraform.tfvars)
if [ $deploy_suffix ]
then
...
npm install
zip -r -j build.zip build/*.js

# TODO: Put test builds in seperate script
